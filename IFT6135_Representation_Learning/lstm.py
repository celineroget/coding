...
class LSTM(nn.Module):
    ...
    def forward(self, inputs, hidden_states):
        ...
        for t in range(sequence_length):
            seq = inputs[:, t, :]
            i = torch.sigmoid(seq @ self.w_ii.t() + self.b_ii + h @ self.w_hi.t() + self.b_hi)
            f = torch.sigmoid(seq @ self.w_if.t() + self.b_if + h @ self.w_hf.t() + self.b_hf)
            g = torch.tanh(seq @ self.w_ig.t() + self.b_ig + h @ self.w_hg.t() + self.b_hg)
            o = torch.sigmoid(seq @ self.w_io.t() + self.b_io + h @ self.w_ho.t() + self.b_ho)
            c = f * c + i * g
            h = o * torch.tanh(c)
            outputs.append(h)
...
class Encoder(nn.Module):
    def __init__(
        ...
        self.rnn = nn.LSTM(embedding_size, hidden_size, batch_first=True, dropout=dropout, bidirectional=True)
...
# Additionner les sorties des directions avant et arrière du LSTM
     #   hidden = hidden.view(2, 1, -1, self.hidden_size)
      #  hidden_forward = hidden[0, :, :]
      #  hidden_backward = hidden[1, :, :]
      #  combined_hidden = torch.sum(torch.stack([hidden_forward, hidden_backward]), dim=0)
