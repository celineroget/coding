...
class LayerNorm(nn.Module):
    ...
    def forward(self, inputs):
        ...
        sigma = torch.std(inputs, dim=-1,correction = 0, keepdim=True)
        outputs = (inputs - mu) / (sigma + self.eps)
...
class MultiHeadedAttention(nn.Module):
    def __init__(self, head_size, num_heads, sequence_length=0):
        ...
        self.W_Q = nn.Linear(self.head_size * self.num_heads, self.head_size * self.num_heads)
        self.W_K = nn.Linear(self.head_size * self.num_heads, self.head_size * self.num_heads)
        self.W_V = nn.Linear(self.head_size * self.num_heads, self.head_size * self.num_heads)
        
        self.W_O = nn.Linear(self.head_size * self.num_heads, self.head_size * self.num_heads)
    ...
    def get_attention_weights(self, queries, keys, mask=None):
        ...
        # TODO: Write your code here
        # ==========================
        attention_weights = torch.matmul(queries, keys.transpose(-2, -1)) / math.sqrt(self.head_size)
        if mask is not None:
            batch_size, seq_len = mask.shape
            attn_mask= mask.view(batch_size, 1, 1, seq_len).expand(batch_size, self.num_heads, seq_len, seq_len)
            attention_weights = attention_weights + -1e9 * (1-attn_mask)
    ...
    def split_heads(self, tensor):
        ...
        # TODO: Write your code here
        # ==========================
        batch_size, sequence_length, x = tensor.shape
        tensor = tensor.reshape(batch_size, sequence_length, self.num_heads, -1)
    ...
    def merge_heads(self, tensor):
        ...
        tensor = tensor.reshape(batch_size, sequence_length, -1)
        return tensor
    ...
    def forward(self, hidden_states, mask=None):
        ...
        # Applying linear projections for Q, K, V
      ...
      # Computing attention scores and applying mask if provided
        attention_weights = self.get_attention_weights(Q, K, mask)

      # Applying attention
        output = self.apply_attention(Q, K, V, mask)
...
class PreNormAttentionBlock(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        x2= self.attn(x1, mask)
        x3 = x + x2
        x4 = self.layer_norm_2(x3)
        x5 = self.linear(x4)
        x6 = x3 + x5
        return x6
...
class Transformer(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        x = self.dropout(x)
        if mask is not None:
          mask_with_cls = torch.cat([mask, torch.ones(mask.size(0), 1, dtype=mask.dtype, device=mask.device)], dim=1)
        else:
          mask_with_cls = None
